...
public class ModPowers {
    public static final PowerFactory<?> POWDER_SNOW_IMMUNE =
            new PowerFactory<>(AddedOriginsReborn.id("powder_snow_immune"),
            new SerializableData(),
            data ->
                    (type, entity) -> new PowderSnowImmune(type, entity))
            .allowCondition();
    public static final PowerFactory<?> FREEZE_IMMUNE =
            new PowerFactory<>(AddedOriginsReborn.id("freeze_immune"),
                    new SerializableData(),
                    data ->
                            (type, entity) -> new FreezeImmune(type, entity))
                    .allowCondition();
    public static final PowerFactory<?> FIRE_RENDERING_IMMUNE =
            new PowerFactory<>(AddedOriginsReborn.id("fire_rendering_immune"),
                    new SerializableData(),
                    data ->
                            (type, entity) -> new FireRenderingImmune(type, entity))
                    .allowCondition();
    public static final PowerFactory<?> MOB_VISION_IMMUNE =
            new PowerFactory<>(AddedOriginsReborn.id("mob_vision_avoid"),
                    new SerializableData().add("mob_condition", ApoliDataTypes.ENTITY_CONDITION),
                    data ->
                            (type, livingEntity) -> new MobVisionImmunity(type, livingEntity, data.get("mob_condition")))
                    .allowCondition();
    public static final PowerFactory<?> FOG_RENDER_CHANGE =
            new PowerFactory<>(AddedOriginsReborn.id("fog_intensity"),
                    new SerializableData()
                            .add("fogStart", SerializableDataTypes.FLOAT, 0.0F)
                            .add("fogEnd", SerializableDataTypes.FLOAT, 16.0F)
                            .add("fogType", SerializableDataTypes.CAMERA_SUBMERSION_TYPE, CameraSubmersionType.NONE),
                    data ->
                            (type, livingEntity) -> new FogIntensity(type, livingEntity,
                                    data.get("fogStart"),
                                    data.get("fogEnd"),
                                    data.get("fogType")))
                    .allowCondition();
    ...
    public static void registerPowers() {
        ...
        Registry.register(ApoliRegistries.POWER_FACTORY, FOG_RENDER_CHANGE.getSerializerId(), FOG_RENDER_CHANGE);
